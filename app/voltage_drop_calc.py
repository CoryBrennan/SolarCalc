...
K = 12.9 ohm-cmil/ft for copper) — a simplified single-point calculation, not
a full per-segment topology model.
"""
...
def _vd_percent(conductor: str, current_a: float, length_ft: float, voltage_v: float) -> tuple[float, float]:
    cm = CIRCULAR_MILS[conductor]
    vd_volts = (math.sqrt(3) * K_COPPER_OHM_CMIL_PER_FT * current_a * length_ft) / cm
...
def check_segment(
    ...
    vd_volts, pct = _vd_percent(conductor, current_a, length_ft, voltage_v)
    ...
    if pct > limit_pct:
        ...
        for candidate in CONDUCTOR_ORDER[idx + 1:]:
            candidate_volts, candidate_pct = _vd_percent(candidate, current_a, length_ft, voltage_v)
